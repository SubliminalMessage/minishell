...
/*   Updated: 2023/06/18 00:52:33 by jre-gonz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_bool	ft_handle_absolute_path(char *cmd)
{
	if (ft_is_dir(cmd))
	{
		ft_printf_fd(2, MINISHELL_ERROR IS_DIR_MSG, cmd);
		return (false);
	}
	if (access(cmd, X_OK) == 0)
		return (true);
	ft_printf_fd(2, MINISHELL_ERROR PERMISSION_DENIED_MSG, cmd);
	return (false);
}
...
static t_bool	ft_handle_cmd(t_cmd *cmd, char **path)
{
	...
	ft_printf_fd(2, MINISHELL_ERROR CMD_NOT_FOUND_MSG, cmd->cmd);
